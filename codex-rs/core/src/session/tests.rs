...
struct QueuedWorkflowContributor {
    snapshots: Arc<
        std::sync::Mutex<
            std::collections::VecDeque<Option<codex_protocol::workflow::WorkflowSnapshot>>,
        >,
    >,
    seen_stores: Arc<std::sync::Mutex<Vec<(String, String)>>>,
}

impl codex_extension_api::WorkflowContributor for QueuedWorkflowContributor {
    fn snapshot<'a>(
        &'a self,
        session_store: &'a codex_extension_api::ExtensionData,
        thread_store: &'a codex_extension_api::ExtensionData,
    ) -> codex_extension_api::WorkflowSnapshotFuture<'a> {
        let snapshots = Arc::clone(&self.snapshots);
        let seen_stores = Arc::clone(&self.seen_stores);
        Box::pin(async move {
            seen_stores
                .lock()
                .expect("workflow seen stores lock")
                .push((
            ...
            snapshots
                .lock()
                .expect("workflow snapshots lock")
                .pop_front()
                .unwrap_or(None)
        })
    }
...
fn workflow_snapshot(
    id: &str,
    state: codex_protocol::workflow::WorkflowStateKind,
) -> codex_protocol::workflow::WorkflowSnapshot {
    codex_protocol::workflow::WorkflowSnapshot {
        id: id.to_string(),
        title: "Regulated workflow".to_string(),
        state,
        boundary_status: codex_protocol::workflow::WorkflowBoundaryStatus::LocalOnly,
        gates: Vec::new(),
...
fn install_queued_workflow_contributor(
    session: &mut Arc<Session>,
    snapshots: Vec<Option<codex_protocol::workflow::WorkflowSnapshot>>,
) -> Arc<std::sync::Mutex<Vec<(String, String)>>> {
    let seen_stores = Arc::new(std::sync::Mutex::new(Vec::new()));
    let contributor = Arc::new(QueuedWorkflowContributor {
        snapshots: Arc::new(std::sync::Mutex::new(snapshots.into())),
        seen_stores: Arc::clone(&seen_stores),
    });
    let mut builder = codex_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    builder.workflow_contributor(contributor);
    Arc::get_mut(session)
        .expect("test should install extensions before cloning session")
        .services
        .extensions = Arc::new(builder.build());
    seen_stores
}

async fn recv_test_event(rx: &async_channel::Receiver<Event>, label: &'static str) -> Event {
...
async fn recv_workflow_status_event(
    rx: &async_channel::Receiver<Event>,
) -> codex_protocol::items::WorkflowStatusItem {
    loop {
        let event = recv_test_event(rx, "expected workflow status event").await;
        if let EventMsg::ItemCompleted(ItemCompletedEvent {
            item: TurnItem::WorkflowStatus(workflow),
            ..
        }) = event.msg
        {
            return workflow;
...
#[tokio::test]
async fn turn_started_event_emits_workflow_status_snapshot() {
    let (mut session, turn_context, rx) = make_session_and_context_with_rx().await;
    let expected_snapshot = workflow_snapshot(
        "workflow-start",
        codex_protocol::workflow::WorkflowStateKind::Running,
    );
    let seen_stores =
        install_queued_workflow_contributor(&mut session, vec![Some(expected_snapshot.clone())]);
    ...
    let first = recv_test_event(&rx, "expected turn started event").await;
    assert!(matches!(
        first.msg,
        EventMsg::TurnStarted(TurnStartedEvent { turn_id, .. }) if turn_id == turn_context.sub_id
    ));

    let second = recv_test_event(&rx, "expected workflow status event").await;
    let EventMsg::ItemCompleted(ItemCompletedEvent {
        ...
        panic!("expected workflow status item completed event");
    };
    assert_eq!(thread_id, session.thread_id);
    assert_eq!(turn_id, turn_context.sub_id);
    assert_eq!(workflow.snapshot, expected_snapshot);
    assert_eq!(
        *seen_stores.lock().expect("workflow seen stores lock"),
        vec![(
...
#[tokio::test]
async fn turn_started_workflow_status_is_persisted_to_rollout_history() {
    let (mut session, turn_context, rx) = make_session_and_context_with_rx().await;
    ...
    session
        ...
        .await;
    recv_test_event(&rx, "expected turn started event").await;
    recv_test_event(&rx, "expected workflow status event").await;
    session
        .flush_rollout()
        .await
        .expect("workflow status rollout should flush");
    ...
    let persisted_workflow = resumed.history.iter().find_map(|item| match item {
        RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
            turn_id,
            item: TurnItem::WorkflowStatus(workflow),
            ..
        })) if turn_id == &turn_context.sub_id => Some(workflow.snapshot.clone()),
        _ => None,
    });
    assert_eq!(persisted_workflow, Some(expected_snapshot));
}

#[tokio::test]
async fn turn_complete_event_emits_updated_workflow_status_snapshot() {
    let (mut session, turn_context, rx) = make_session_and_context_with_rx().await;
    let start_snapshot = workflow_snapshot(
        "workflow-start",
        codex_protocol::workflow::WorkflowStateKind::Running,
    );
    let complete_snapshot = workflow_snapshot(
        "workflow-complete",
        codex_protocol::workflow::WorkflowStateKind::Completed,
    );
    install_queued_workflow_contributor(
        &mut session,
        vec![Some(start_snapshot), Some(complete_snapshot.clone())],
    );
    ...
    session
        ...
        .await;
    recv_test_event(&rx, "expected turn started event").await;
    recv_test_event(&rx, "expected start workflow status event").await;
    ...
    let complete_event = recv_test_event(&rx, "expected turn complete event").await;
    assert!(matches!(
        complete_event.msg,
        EventMsg::TurnComplete(TurnCompleteEvent { turn_id, .. }) if turn_id == turn_context.sub_id
    ));

    let workflow_event = recv_test_event(&rx, "expected complete workflow status event").await;
    let EventMsg::ItemCompleted(ItemCompletedEvent {
        item: TurnItem::WorkflowStatus(workflow),
        ..
    }) = workflow_event.msg
    ...
    assert_eq!(workflow.snapshot, complete_snapshot);
}

#[tokio::test]
async fn turn_abort_lifecycle_emits_workflow_status_after_abort_contributors() {
    struct AbortMarker;

    struct AbortAwareWorkflowContributor;

    impl codex_extension_api::TurnLifecycleContributor for AbortAwareWorkflowContributor {
        fn on_turn_abort<'a>(
            &'a self,
            input: codex_extension_api::TurnAbortInput<'a>,
        ) -> codex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                input.thread_store.insert(AbortMarker);
            })
        }
    }

    impl codex_extension_api::WorkflowContributor for AbortAwareWorkflowContributor {
        fn snapshot<'a>(
            &'a self,
            ...
            Box::pin(async move {
                let state = if thread_store.get::<AbortMarker>().is_some() {
                    codex_protocol::workflow::WorkflowStateKind::Blocked
                } else {
                    codex_protocol::workflow::WorkflowStateKind::Running
                };
                Some(workflow_snapshot("workflow-abort", state))
            })
        }
    }

    let (mut session, turn_context, rx) = make_session_and_context_with_rx().await;
    let contributor = Arc::new(AbortAwareWorkflowContributor);
    let mut builder = codex_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let workflow = recv_workflow_status_event(&rx).await;
    assert_eq!(
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let session = Session {
        ...
        services,
        workspace_context_only_memory_tainted: std::sync::atomic::AtomicBool::new(false),
