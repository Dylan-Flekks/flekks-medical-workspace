...
- Experimental `modelToolMode: "isolated"` is an implementation-in-progress bounded-analysis path, not a general security boundary. Creation requires a fresh ephemeral thread, an explicit model, disabled provider fallback, empty environments/runtime roots, and no instructions, tools, capabilities, history, cwd, permission, service, personality, or multi-agent overrides. The core runtime remains unpublished until the live creator connection is recorded as its exclusive owner; status and lifecycle notifications are routed only to that connection, `thread/unsubscribe` is rejected, and disconnect makes all of that connection's isolated threads unreachable before bounded shutdown waits begin. Provider auth commands, environment credentials/headers, configured bearer tokens, AWS auth, and provider query/header overrides are rejected or stripped; OpenAI auth must already exist as a cached snapshot, and isolated 401 responses never invoke auth recovery. App-server request/response analytics for the permitted isolated `thread/start` and `turn/start` path, plus its turn-notification analytics, are suppressed. Do not enable this mode for sensitive data yet: comprehensive direct API rejection, provider/model/endpoint and server-owned-schema provisioning, and remaining runtime trace/log isolation are still follow-up requirements.
...
- `workspace/draft/checkpoint/create`, `workspace/draft/checkpoint/list`, `workspace/draft/session/list`, `workspace/draft/session/close` — experimental local-only draft recovery APIs. The first schema-versioned checkpoint creates a patient-scoped session; later checkpoints are immutable revisions with server-computed hashes, and identical content replays the original checkpoint. Session pages include the exact `currentCheckpoint`, even when replay moves `currentRevision` to an older revision. Lists use opaque cursors, and close/discard is durable and terminal. `actor` records attribution but is not authentication; encounter and note IDs are correlation metadata, not chart-read or chart-write authority. These APIs do not create canonical notes, launch an agent, or submit a context packet.
- `workspace/guide/run/start`, `workspace/guide/run/finish`, `workspace/guide/run/list` — experimental local-only Workspace Guide provenance APIs. `start` persists and returns a bounded, immutable request envelope bound to the exact current draft checkpoint; it does not launch a model or agent. Request envelopes are limited to 32 KiB and terminal envelopes to 16 KiB. Persisted scalar provenance is also byte-bounded: idempotency keys, actors, models, and source thread/turn IDs are limited to 256 bytes; triggers and providers are limited to 128 bytes. An exact idempotent replay returns the original run even if its checkpoint is now stale, while a new key against that old checkpoint fails as `staleCheckpoint`. Every run is read-only, reports `modelToolMode: "disabled"`, and has no canonical chart-write authority. `finish` records a tagged completed, failed, or canceled terminal envelope plus the supplied thread/turn provenance; it does not independently prove those IDs exist. `list` is patient-scoped and uses opaque cursors while deriving whether each source checkpoint is stale. Stored request and terminal hashes are recomputed on every state read and API mapping so corruption fails closed. Semantic failures expose stable `error.data.kind` values (`validation`, `staleCheckpoint`, `idempotencyConflict`, `activeRunConflict`, or `terminalConflict`); malformed wire requests remain generic JSON-RPC errors. `actor` records attribution but is not authentication. These APIs never create or edit canonical notes, context packets, agent results, or note proposals.
- `workspace/chart/commit` — experimental atomic chart-save API. Supply an `idempotencyKey`, clinician `actor` and non-empty `reason`, optional source thread/turn provenance, and at most one optional `client`, `safetyItem`, `encounter`, `note`, `document`, `artifactDerivative`, `contextClip`, and `task` upsert. A new patient supplies `client` without `clientId`; an existing-patient record-only save supplies `clientId` without a demographic snapshot; a full demographic edit may supply `client` with its existing ID and omit `clientId`, or supply both with matching IDs. Omitting both patient identifiers or sending mismatched IDs is rejected. This identity-only form prevents note or task saves from overwriting newer demographic, contact, or coverage data. Child `clientId` values are checked against the patient root and normalized by the store; leave same-commit relation IDs blank or null to link server-allocated encounter, note, document, derivative, and clip IDs. Every returned mutable chart entity other than a note includes an opaque `version` token. When editing an existing client, safety item, encounter, document, artifact derivative, context clip, or task, echo its token in the corresponding camelCase `expectedVersions` field; stale tokens fail without writes as `error.data.kind: "staleEntityVersion"` with entity and expected/actual token details. Existing note edits instead include `note.expectedBaseRevision`; signed notes remain immutable. The response returns the durable commit ID, replay status, changed entity kinds, all submitted records in their final persisted form, the resulting note revision, and commit time. Reusing an idempotency key with different content fails closed. Semantically valid requests that reach the chart commit operation expose stable `error.data.kind` values (`validation`, `staleNoteRevision`, `staleEntityVersion`, or `idempotencyConflict`), with conflict details where applicable; malformed JSON or requests missing required wire fields use the generic JSON-RPC invalid-request error (`-32600`) without this typed chart error contract.
...
- An isolated thread accepts exactly one `turn/start`. The request must contain exactly one nonempty plain-text input of at most 32 KiB and provide a strict object `outputSchema`; client IDs, metadata, context, environment, cwd, permission, model, reasoning, personality, collaboration, and multi-agent overrides are rejected. The creator connection remains analytics-quarantined, including when a turn references a stale or mistyped thread id. Invalid input or schema fails synchronously without consuming the one valid turn. The response request advertises no tools or parallel tool calls, and model output must be one schema-valid JSON object.
