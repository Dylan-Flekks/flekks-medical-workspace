...
- `modelToolMode` thread-start note — `"disabled"` is a persistent thread mode. The
  `"workspaceContextOnly"` value is deliberately rejected by `thread/start`; it is available only
  as a one-turn `turn/start` override.
...
- `thread/memoryMode/set` — experimental; set a thread’s persisted memory eligibility to `"enabled"` or `"disabled"` for either a loaded thread or a stored rollout; returns `{}` on success. Enabling is permanently rejected after the thread has persisted a `workspaceContextOnly` turn.
...
- `modelToolMode` medical-turn note — `"workspaceContextOnly"` is a one-turn boundary. It requires
  ...
  the submitted prompt and tool argument/result payloads.
