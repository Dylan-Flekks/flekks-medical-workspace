...
- `workspace/context/packet/create`, `workspace/agent/run/start`, `workspace/agent/run/list`, `workspace/agent/run/status/update`, `workspace/agent/run/context/read`, `workspace/agent/run/source/list` — experimental medical-agent provenance APIs. A context packet is prepared against an explicit patient, note, and base revision. Callers may omit draft provenance for the legacy/manual flow, or provide all of `sourceDraftSessionId`, `sourceDraftCheckpointId`, `sourceDraftCheckpointRevision`, and `sourceDraftCheckpointSha256` to bind the packet to the exact current reviewed checkpoint and its matching `contextEnvelopeJson.sourceCheckpoint`; partial, stale, cross-scope, or mismatched bindings fail closed, while an exact retry returns the existing packet instead of duplicating it. Packet responses retain the four source fields for audit. These identifiers are provenance and correlation data, not authentication or chart-write authority. Starting an idempotent run submits that packet and hashes its authorization/output contract. Context reads derive patient ownership from the run, enforce its explicit `visitHistory`/`progressNotes` scope, bound returned note content, redact local-path tokens, and record every returned snapshot and hash for later audit. The model-facing `workspace_context_read` tool delegates to this same state-enforced reader.
