...
- `workspace/draft/checkpoint/create`, `workspace/draft/checkpoint/list`, `workspace/draft/session/list`, `workspace/draft/session/close` — experimental local-only draft recovery APIs. The first schema-versioned checkpoint creates a patient-scoped session; later checkpoints are immutable revisions with server-computed hashes, and identical content replays the original checkpoint. Snapshot `schemaVersion: 1` remains compatible for patient/note recovery. Snapshot `schemaVersion: 2` may additionally checkpoint client-authored agent-request and context-selection metadata as opaque recovery data; accepting that metadata does not infer clinical content, launch a model, submit a context packet, or mutate the canonical chart. A new client may send a trimmed `sessionCreationKey` of at most 256 bytes instead of `sessionId`; retrying that key for the same patient resolves the same session, so a lost first response is safe to retry. Reusing the key with changed draft content appends the next revision, while a terminal keyed session rejects further checkpoints. `sessionId` and `sessionCreationKey` are mutually exclusive, and omitting both preserves legacy random-session creation. Creation keys are patient-scoped storage metadata and are never returned by list responses or written to audit events. Session pages include the exact `currentCheckpoint`, even when replay moves `currentRevision` to an older revision. `workspace/draft/session/list` requires exactly one scope: provide `clientId` for the existing patient-scoped behavior, or omit it and pass `allClients: true` to discover active drafts across active, non-archived patients. Global discovery rejects `includeClosed`, returns at most one full checkpoint per page because a draft may be 1 MiB, and continues through the same opaque `(updatedAt, id)` keyset cursor. TUI clients must check `uses_remote_workspace()` and reject global discovery before sending the request; this API does not itself prove that its transport is process-local. Lists use opaque cursors, and close/discard is durable and terminal. Close accepts the optional all-or-none `expectedCurrentCheckpointId`, `expectedCurrentCheckpointRevision`, and `expectedCurrentCheckpointSha256` tuple; recovery and canonical-save clients must echo all three values from `currentCheckpoint` so a concurrently newer checkpoint fails closed before either a transition or same-status replay. Omission remains supported for legacy callers. `actor` records attribution but is not authentication; encounter and note IDs are correlation metadata, not chart-read or chart-write authority. These APIs do not create canonical notes, launch an agent, or submit a context packet.
