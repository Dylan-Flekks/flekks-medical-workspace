...
- `thread/start`, `thread/resume`, and `thread/fork` responses include the legacy `sandbox` compatibility projection. `instructionSources` lists loaded instruction files using each source environment's native absolute path syntax, including files loaded from remote environments. Experimental clients can read `runtimeWorkspaceRoots` for the thread-scoped runtime roots, `activePermissionProfile` for the named or implicit built-in profile identity/provenance when known, and `modelToolMode` for the effective sticky regular-sampling mode. Their deprecated experimental `multiAgentMode` field, and the corresponding thread setting, always report `explicitRequestOnly`; Ultra reasoning effort is the source of proactive multi-agent behavior.
...
- `thread/memoryMode/set` — experimental; set a thread’s persisted memory eligibility to `"enabled"` or `"disabled"` for either a loaded thread or a stored rollout; returns `{}` on success. Enabling is permanently rejected after the thread has persisted a `workspaceContextOnly` or `workspacePlanningOnly` turn.
...
- `workspace/task/list`, `workspace/task/upsert`, `workspace/task/status/update` — experimental local workspace task APIs. Tasks are scoped by `clientId`, may link to optional encounter, note, and document IDs, and status updates return `task: null` when the task ID does not belong to that client. Task create/update/status changes are audit recorded. `workspace/context/get` includes compact open task summaries for the selected client.
- `workspace/client/list`, `workspace/client/get`, `workspace/client/upsert` — experimental local patient APIs. In addition to the retained `displayName`, legacy `sexOrGender`, `email`, and primary-coverage projection, patient records expose structured legal-name components, previous and preferred names, administrative sex, preferred language/interpreter need, labeled primary/secondary phones, primary/secondary email, mailing address, and emergency-contact fields. Existing-patient updates must echo the opaque client `version` as `expectedVersion`; omitted new demographic fields are preserved, while an explicit `null` clears a field. `primaryEmail` and the legacy `email` alias must agree when both are supplied. Legacy coverage fields can initialize a new patient's primary projection but demographic updates cannot rewrite normalized coverage. This administrative data remains local and is not automatically added to an agent context packet.
- `workspace/coverage/list` — experimental cursor-paged coverage API ordered by priority 1–3 (primary, secondary, tertiary). Each independently versioned record separates patient, beneficiary, subscriber, policy, and subscriber-address data and includes derived `billingReadiness` (`match`, `mismatch`, `unverified`, `stale`, or `incomplete`). Legacy client coverage fields remain a compatibility view of priority 1; once a primary record is managed through the structured API, legacy writers cannot rewrite or delete it. Coverage readiness is advisory for billing workflows and never blocks clinical chart saves.
- `workspace/coverage/verification/list`, `workspace/coverage/verification/create` — experimental append-only, human-confirmed insurance-card comparison APIs. Creation requires a current `expectedPatientVersion`, `expectedCoverageVersion`, and `expectedDocumentVersion`, plus an active patient-owned, patient-scoped, present local card reference whose insurance-card kind and SHA-256 content snapshot can be validated. The clinician enters the printed name/member ID; the server does not run OCR, vision, eligibility, or payer communication. The server deterministically compares beneficiary or subscriber identity, records per-field mismatches and immutable patient/coverage/document version and card-content pins, validates Medicare beneficiary IDs against the 11-character CMS format, and derives staleness when relevant identity changes. `actor` is audit attribution, not authentication.
- `workspace/dataPolicy/read`, `workspace/dataPolicy/provision` — experimental synthetic-workspace policy APIs. `read` returns the durable one-way data classification plus whether this app-server process was launched with authority to provision an empty store. Provisioning authority is derived and snapshotted by the server at startup: `FLEKKS_MEDICAL_WORKSPACE_DATA_CLASSIFICATION` must equal `synthetic` exactly, `CODEX_SQLITE_HOME` must be absolute and resolve to the effective `sqlite_home`, and that path must not resolve to `CODEX_HOME`. Requests accept no classification, path, actor, or provenance. `provision` records a fixed app-server source and can only change the singleton policy from `unclassified` to `synthetic` while every known workspace table is empty; exact retries return `alreadySynthetic`. Unknown tables, policy corruption, nonempty stores, and launch-configuration drift fail closed. The environment enables the transition but is not authentication; the persisted immutable policy remains the database's durable classification.
- `workspace/draft/checkpoint/create`, `workspace/draft/checkpoint/list`, `workspace/draft/session/list`, `workspace/draft/session/close` — experimental local-only draft recovery APIs. Draft schema versions 1 and 2 are accepted; version 2 adds the typed medical `contextPlan` while version 1 remains recoverable through the compatibility migration. The first schema-versioned checkpoint creates a patient-scoped session and must omit current-head provenance. Every existing-session append must echo the all-or-none `expectedCurrentCheckpointId`, `expectedCurrentCheckpointRevision`, and `expectedCurrentCheckpointSha256` tuple from the observed `currentCheckpoint`; a concurrently newer head fails closed. Checkpoints are immutable revisions with server-computed hashes. Retrying the exact append that already advanced the head replays its checkpoint, while reusing older content cannot move `currentRevision` backward. Lists use opaque cursors, and close/discard is durable and terminal. Close accepts the same tuple; recovery and canonical-save clients should echo all three values so a concurrently newer checkpoint fails closed before either a transition or same-status replay. Close omission remains supported for legacy callers. `actor` records attribution but is not authentication; encounter and note IDs are correlation metadata, not chart-read or chart-write authority. These APIs do not create canonical notes, launch an agent, or submit a Context Plan.
- `workspace/plan/session/open`, `workspace/plan/session/get`, `workspace/plan/session/bindThread`, `workspace/plan/snapshot/get`, `workspace/plan/recovery/get`, `workspace/plan/guideRun/start`, `workspace/plan/guideRun/finish`, `workspace/plan/message/append`, `workspace/plan/revision/outdate`, `workspace/plan/revision/submit`, `workspace/plan/proposal/resolve` — experimental, opt-in APIs for the persistent patient Plan conversation. Every session and child record is checked against `clientId`; a session binds once to one dedicated source thread and cannot be rebound to another patient or session. `snapshot/get` returns messages after an optional sequence plus recent revisions, immutable submission receipts, and proposals, with server-enforced limits (messages 1–500, revisions 1–100, proposals 1–200). `recovery/get` reconciles active guide runs and returns pending questions, the current revision, and the last durable completion receipt. `guideRun/finish` is a recovery-only terminalization surface that accepts failed or canceled outcomes; successful turns can complete only through core's atomic planning-turn transaction. Clients may append only unclaimed clinician messages without model provenance. Core owns assistant records, while state terminalization creates deterministic system-attributed error records in the same transaction as a failed or canceled planning run. Core persists assistant messages and any publishable Plan revision atomically with the guide-run completion and an ordered, hashed evidence manifest. Plan content and evidence are immutable; only revision status can move from `current` to `outdated` or `submitted`. Proposal creation is not a public method; stored evidence-linked note-revision, note-addendum, or task drafts expose only clinician accept/decline resolution, which records the review decision and never changes the canonical chart. `revision/submit` is the explicit human-controlled transition: it requires the exact packet ID, agent-run ID, checkpoint identity and hash, Plan content hash, and evidence-manifest binding carried by the submitted packet and run; stale or mismatched bindings fail closed. The server records a human actor for proposal resolution and submission, but that attribution is not authentication.
- `workspace/chart/commit` — experimental atomic chart-save API. Supply an `idempotencyKey`, clinician `actor` and non-empty `reason`, optional source thread/turn provenance, and at most one optional `client`, `coverage`, `safetyItem`, `encounter`, `note`, `document`, `artifactDerivative`, `contextClip`, and `task` upsert. A new patient supplies `client` without `clientId`; an existing-patient record-only save supplies `clientId` without a demographic snapshot; a full demographic edit may supply `client` with its existing ID and omit `clientId`, or supply both with matching IDs. Omitting both patient identifiers or sending mismatched IDs is rejected. This identity-only form prevents note or task saves from overwriting newer demographic, contact, or coverage data. Child `clientId` values are checked against the patient root and normalized by the store; leave same-commit relation IDs blank or null to link server-allocated coverage, encounter, note, document, derivative, and clip IDs. Every returned mutable chart entity other than a note includes an opaque `version` token. When editing an existing client, coverage, safety item, encounter, document, artifact derivative, context clip, or task, echo its token in the corresponding camelCase `expectedVersions` field; stale tokens fail without writes as `error.data.kind: "staleEntityVersion"` with entity and expected/actual token details. Existing note edits instead include `note.expectedBaseRevision`; signed notes remain immutable. The response returns the durable commit ID, replay status, changed entity kinds, all submitted records in their final persisted form, the resulting note revision, and commit time. Reusing an idempotency key with different content fails closed. Semantically valid requests that reach the chart commit operation expose stable `error.data.kind` values (`validation`, `staleNoteRevision`, `staleEntityVersion`, or `idempotencyConflict`), with conflict details where applicable; malformed JSON or requests missing required wire fields use the generic JSON-RPC invalid-request error (`-32600`) without this typed chart error contract.
- `workspace/context/packet/create`, `workspace/agent/run/start`, `workspace/agent/run/list`, `workspace/agent/run/status/update`, `workspace/agent/run/source/list` — experimental medical-agent provenance APIs. New medical Context Plans identify `workspaceProfile: "medical"`, `planSchemaVersion: 1`, and an exact immutable draft checkpoint through `sourceCheckpointId` plus its server-computed `sourceCheckpointSha256`. They also provide `readinessJson`, whose versioned warnings and clinician acknowledgements are bound to that same checkpoint hash. Packet creation verifies patient/note/base-revision ownership and embeds this metadata into the envelope before computing its hash. Submission and run start revalidate that the checkpoint is still the session head and reject unsupported schemas, malformed readiness data, stale or mismatched checkpoints, and unacknowledged warnings. Legacy callers that omit all five fields receive an explicitly marked replay-compatible legacy readiness record; partially specified metadata fails closed. Starting an idempotent run submits the packet and hashes its authorization/output contract. Source listing is an audit surface, not model context authority; its canonical handoff-prompt record retains the durable hash and metadata but redacts the prompt snapshot at the app-server boundary.
- `workspace/agent/run/context/read` — experimental denial-only compatibility endpoint. Run-id-only context reads always fail closed. Additional `visitHistory`/`progressNotes` context is available only to the exact claimed `workspaceContextOnly` model turn through `workspace_context_read`; that bound tool derives patient ownership from the run, enforces the packet scope, bounds returned note content, redacts local-path tokens, and records every returned snapshot and hash for later audit.
- `workspace/agent/result/create`, `workspace/note/proposal/create`, `workspace/note/proposal/resolve`, `workspace/note/proposal/decision/list` — experimental human-review APIs. `workspace/agent/result/create` accepts clinician-attributed manual imports only; supplying `runId` is rejected because bound model output is committed exclusively by core's exact turn-completion transaction. Agent output remains non-canonical until it becomes a revision-bound proposal and a clinician explicitly accepts it. Results, proposals, and append-only decisions retain their packet/run provenance; stale proposals cannot overwrite a newer canonical note revision.
...
- `turn/start` — add user input to a thread and begin Codex generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. `clientUserMessageId` is optional; when supplied, the corresponding `userMessage` item echoes it as `clientId`. Experimental `runtimeWorkspaceRoots` replaces the thread-scoped runtime workspace roots used to materialize `:workspace_roots`; paths must be absolute. Prefer experimental `permissions` profile selection by id for permission overrides; the legacy `sandboxPolicy` field is still accepted but cannot be combined with `permissions`. Experimental `modelToolMode: "disabled"` overrides the sticky tool setting for this and subsequent regular sampling turns; omit it to inherit the thread value. The controlled `"workspaceContextOnly"` and `"workspacePlanningOnly"` modes apply to exactly this turn and restore the prior persistent mode during turn setup. A mode change is rejected while a regular turn or realtime conversation is active. Effective settings are reported by `thread/settings/updated`. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode". Deprecated experimental `multiAgentMode` is ignored; Ultra reasoning effort selects proactive behavior.
- `modelToolMode` medical-turn note — `"workspaceContextOnly"` is a one-turn boundary. It requires
  a persisted root thread and exactly one canonical generated packet prompt containing a
  `- run_id: ...` handle. Before sampling, SQLite atomically claims that run for the exact thread,
  turn, provider, model, packet hash, and prompt; a second or altered claim is rejected. The mode
  exposes only the non-namespaced `workspace_context_read` function bound to that claimed execution,
  rechecks the claim before every model-facing chart read, sends only current-turn transcript items to
  the model, rejects inline image/skill/plugin/mention inputs, skips skill/plugin/extension context
  and lifecycle or tool hooks, rejects every other tool-shaped output, and restores the prior live
  persistent mode before the restricted turn executes. Before sampling, it durably marks the
  thread as permanently ineligible for memory generation and schedules removal of any selected
  prior memory; later attempts to re-enable memory are rejected. A process resume or fork also
  reconstructs the tool mode as `"disabled"` rather than inheriting an earlier permissive mode. Active restricted turns reject
  steering, realtime startup, and forking. Generic session logs, tool traces, and telemetry redact
  the submitted prompt and tool argument/result payloads. The final Assistant message is withheld
  from client streaming and rollout history until core atomically verifies the submitted Plan
  receipt and packet/run identity, writes the review-pending result plus body hash and exact
  message/thread/turn/provider/model receipt, and completes the run. A failed commit emits no
  authoritative Agent Review response.
- `modelToolMode` planning-turn note — `"workspacePlanningOnly"` requires a persisted root thread in
  Plan collaboration mode and a claimed guide run for one patient, Plan session, and immutable draft
  checkpoint. Its plain-text input must carry the exact run, Plan-session, patient, checkpoint ID,
  checkpoint revision, and checkpoint hash contract. The model sees only verified turns from the
  dedicated planning conversation, exposes only the bound `workspace_context_read` function, and
  shares the restricted mode's memory-disable, hook suppression, payload-redaction, no-steering,
  no-realtime, and no-fork boundaries. Clarifying questions are ordinary persisted assistant messages
  answered in a later Plan turn. A final assistant message is withheld from both the client and rollout
  history until core atomically completes the guide run and persists the assistant message, its ordered
  evidence manifest, and any strict `workspace_plan_artifact`. An artifact publishes a new immutable
  Plan revision only when it contains exactly `planMarkdown`, string-array `decisions`, and an empty
  `openQuestions` array; conversational responses without an artifact do not create a revision.
...
Experimental: use `thread/memoryMode/set` to change whether a thread remains eligible for future memory generation. A thread that has persisted a `workspaceContextOnly` turn is permanently ineligible and rejects `"enabled"`.
