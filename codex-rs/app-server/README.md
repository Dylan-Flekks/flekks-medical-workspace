...
- `workspace/draft/checkpoint/create`, `workspace/draft/checkpoint/list`, `workspace/draft/session/list`, `workspace/draft/session/close` — experimental local-only draft recovery APIs. The first schema-versioned checkpoint creates a patient-scoped session and must omit current-head provenance. Every existing-session append must echo the all-or-none `expectedCurrentCheckpointId`, `expectedCurrentCheckpointRevision`, and `expectedCurrentCheckpointSha256` tuple from the observed `currentCheckpoint`; a concurrently newer head fails closed. Checkpoints are immutable revisions with server-computed hashes. Retrying the exact append that already advanced the head replays its checkpoint, while reusing older content cannot move `currentRevision` backward. Lists use opaque cursors, and close/discard is durable and terminal. Close accepts the same tuple; recovery and canonical-save clients should echo all three values so a concurrently newer checkpoint fails closed before either a transition or same-status replay. Close omission remains supported for legacy callers. `actor` records attribution but is not authentication; encounter and note IDs are correlation metadata, not chart-read or chart-write authority. These APIs do not create canonical notes, launch an agent, or submit a context packet.
