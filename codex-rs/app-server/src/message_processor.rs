...
use crate::request_processors::WorkspaceRequestProcessor;
...
pub(crate) struct MessageProcessor {
    ...
    workspace_processor: WorkspaceRequestProcessor,
...
impl MessageProcessor {
    ...
    async fn handle_initialized_client_request(
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::WorkspaceClientList { params, .. } => {
                self.workspace_processor.client_list(params).await
            }
            ClientRequest::WorkspaceClientGet { params, .. } => {
                self.workspace_processor.client_get(params).await
            }
            ClientRequest::WorkspaceClientUpsert { params, .. } => {
                self.workspace_processor.client_upsert(params).await
            }
            ...
            ClientRequest::WorkspacePracticeLibraryList { params, .. } => {
                self.workspace_processor.practice_library_list(params).await
            }
            ClientRequest::WorkspaceDocumentGet { params, .. } => {
                self.workspace_processor.document_get(params).await
            }
            ClientRequest::WorkspaceDocumentUpsert { params, .. } => {
                self.workspace_processor.document_upsert(params).await
            }
            ClientRequest::WorkspaceDocumentArchive { params, .. } => {
                self.workspace_processor.document_archive(params).await
            }
            ClientRequest::WorkspacePatientSafetyItemList { params, .. } => {
                self.workspace_processor
                    .patient_safety_item_list(params)
                    .await
            }
            ClientRequest::WorkspacePatientSafetyItemUpsert { params, .. } => {
                self.workspace_processor
                    .patient_safety_item_upsert(params)
                    .await
            }
            ClientRequest::WorkspacePatientSafetyItemArchive { params, .. } => {
                self.workspace_processor
                    .patient_safety_item_archive(params)
                    .await
            }
            ClientRequest::WorkspaceArtifactDerivativeList { params, .. } => {
                self.workspace_processor
                    .artifact_derivative_list(params)
                    .await
            }
            ClientRequest::WorkspaceArtifactDerivativeUpsert { params, .. } => {
                self.workspace_processor
                    .artifact_derivative_upsert(params)
                    .await
            }
            ClientRequest::WorkspaceArtifactDerivativeStatusUpdate { params, .. } => {
                self.workspace_processor
                    .artifact_derivative_status_update(params)
                    .await
            }
            ClientRequest::WorkspaceContextClipList { params, .. } => {
                self.workspace_processor.context_clip_list(params).await
            }
            ClientRequest::WorkspaceContextClipUpsert { params, .. } => {
                self.workspace_processor.context_clip_upsert(params).await
            }
            ClientRequest::WorkspaceContextClipStatusUpdate { params, .. } => {
                self.workspace_processor
                    .context_clip_status_update(params)
                    .await
            }
            ClientRequest::WorkspaceTaskList { params, .. } => {
                self.workspace_processor.task_list(params).await
            }
            ClientRequest::WorkspaceTaskUpsert { params, .. } => {
                self.workspace_processor.task_upsert(params).await
            }
            ClientRequest::WorkspaceTaskStatusUpdate { params, .. } => {
                self.workspace_processor.task_status_update(params).await
            }
            ClientRequest::WorkspaceEncounterList { params, .. } => {
                self.workspace_processor.encounter_list(params).await
            }
            ClientRequest::WorkspaceEncounterUpsert { params, .. } => {
                self.workspace_processor.encounter_upsert(params).await
            }
            ...
            ClientRequest::WorkspaceContextPacketCreate { params, .. } => {
                self.workspace_processor.context_packet_create(params).await
            }
            ...
            ClientRequest::WorkspaceAgentResultCreate { params, .. } => {
                self.workspace_processor.agent_result_create(params).await
            }
            ClientRequest::WorkspaceAgentResultStatusUpdate { params, .. } => {
                self.workspace_processor
            ...
            ClientRequest::WorkspaceNoteGet { params, .. } => {
                self.workspace_processor.note_get(params).await
            }
            ClientRequest::WorkspaceNoteUpsert { params, .. } => {
                self.workspace_processor.note_upsert(params).await
            }
            ClientRequest::WorkspaceNoteArchive { params, .. } => {
                self.workspace_processor.note_archive(params).await
            }
            ClientRequest::WorkspaceNoteSign { params, .. } => {
                self.workspace_processor.note_sign(params).await
            }
            ClientRequest::WorkspaceNoteSignatureList { params, .. } => {
                self.workspace_processor.note_signature_list(params).await
            }
            ClientRequest::WorkspaceNoteAddendumCreate { params, .. } => {
                self.workspace_processor.note_addendum_create(params).await
            }
            ClientRequest::WorkspaceNoteAddendumList { params, .. } => {
                self.workspace_processor.note_addendum_list(params).await
            }
            ClientRequest::WorkspaceNoteProposalList { params, .. } => {
                self.workspace_processor.proposal_list(params).await
            }
            ClientRequest::WorkspaceNoteProposalCreate { params, .. } => {
                self.workspace_processor.proposal_create(params).await
            }
