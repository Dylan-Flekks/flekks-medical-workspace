...
fn event_msg_persistence_mode(ev: &EventMsg) -> Option<EventPersistenceMode> {
    match ev {
        ...
        EventMsg::ItemCompleted(event) => {
            // Plan and workflow-status items are derived runtime status, not
            // raw ResponseItem history, so persist their completion to replay
            // them on resume without bloating rollouts with every item lifecycle.
            if matches!(
                event.item,
                codex_protocol::items::TurnItem::Plan(_)
                    | codex_protocol::items::TurnItem::WorkflowStatus(_)
            ) {
                Some(EventPersistenceMode::Limited)
            } else {
                None
...
mod tests {
    ...
    use codex_protocol::workflow::WorkflowBoundaryStatus;
    use codex_protocol::workflow::WorkflowSnapshot;
    use codex_protocol::workflow::WorkflowStateKind;

    #[test]
    fn workflow_status_item_completed_persists_in_limited_mode() {
        let event = EventMsg::ItemCompleted(ItemCompletedEvent {
            thread_id: ThreadId::new(),
            turn_id: "turn-1".to_string(),
            item: TurnItem::WorkflowStatus(WorkflowStatusItem {
                id: "workflow-status-1".to_string(),
                snapshot: WorkflowSnapshot {
                    id: "workflow-1".to_string(),
                    title: "Regulated workflow".to_string(),
                    state: WorkflowStateKind::Running,
