...
See the Codex keymap documentation for supported actions and examples."
    ...
    async fn handle_workspace_dashboard_event(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut AppServerSession,
        event: TuiEvent,
        ...
        match event {
            TuiEvent::Key(key_event) => {
                ...
                let action = self
                    .workspace_dashboard
                    .as_mut()
                    .map(|dashboard| dashboard.handle_key_event_for_viewport(key_event, viewport))
            ...
            TuiEvent::Paste(pasted) => {
                ...
                let action = self
                    .workspace_dashboard
                    .as_mut()
                    .map(|dashboard| dashboard.handle_paste(pasted))
                    .unwrap_or(WorkspaceDashboardAction::Consumed);
                self.handle_workspace_dashboard_action(tui, app_server, action)
                    .await;
            }
            TuiEvent::MouseScroll(scroll_event) => {
                ...
                let action = self
                    .workspace_dashboard
                    .as_mut()
                    .map(|dashboard| dashboard.handle_mouse_scroll(scroll_event, viewport))
                    .unwrap_or(WorkspaceDashboardAction::Consumed);
                self.handle_workspace_dashboard_action(tui, app_server, action)
                    .await;
            }
    ...
    async fn handle_workspace_dashboard_action(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut AppServerSession,
        ...
        match action {
            WorkspaceDashboardAction::CreateAddendum {
                ...
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .save_current_addendum(app_server, note_id, base_revision, body)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget
                        .add_error_message(format!("Failed to save workspace addendum: {err}"));
                }
            }
            ...
            WorkspaceDashboardAction::ResolveProposal {
                ...
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .resolve_note_proposal(app_server, proposal_id, accept)
                        .await
                } else {
                    Ok(())
                };
            ...
            WorkspaceDashboardAction::SaveAgentResult { packet_id, body } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .save_agent_result(app_server, packet_id, body)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget
                        .add_error_message(format!("Failed to save workspace agent result: {err}"));
                }
            }
            WorkspaceDashboardAction::AssociatePracticeLibraryDocument(params) => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .associate_practice_library_document(app_server, *params)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget.add_error_message(format!(
                        "Failed to associate workspace practice record: {err}"
                    ));
                }
            }
            WorkspaceDashboardAction::ImportDocumentToVault { document_id } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .import_document_to_vault(app_server, document_id)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget.add_error_message(format!(
                        "Failed to import workspace file to local vault: {err}"
                    ));
                }
            }
            WorkspaceDashboardAction::ChoosePatientFiles => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard.choose_patient_files(app_server).await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget
                        .add_error_message(format!("Failed to choose patient files: {err}"));
                }
            }
            WorkspaceDashboardAction::GenerateDocumentThumbnail { document_id } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .generate_document_thumbnail(app_server, document_id)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget.add_error_message(format!(
                        "Failed to generate workspace file preview: {err}"
                    ));
                }
            }
            WorkspaceDashboardAction::OpenDocumentFile { document_id } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard.open_document_file(document_id)
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget
                        .add_error_message(format!("Failed to open workspace file: {err}"));
                }
            }
            WorkspaceDashboardAction::SaveDroppedFileReferences(params) => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .save_dropped_file_references(app_server, params)
                        .await
                } else {
                    Ok(())
                };
            ...
            WorkspaceDashboardAction::UpdateAgentResultStatus { result_id, status } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .update_agent_result_status(app_server, result_id, status)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget.add_error_message(format!(
                        "Failed to update workspace agent result: {err}"
                    ));
                }
            }
            WorkspaceDashboardAction::UpdateArtifactDerivativeStatus {
                ...
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .update_artifact_derivative_status(app_server, derivative_id, review_status)
                        .await
                } else {
                    Ok(())
                };
            ...
            WorkspaceDashboardAction::UpdateContextClipStatus {
                ...
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .update_context_clip_status(app_server, clip_id, review_status)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget.add_error_message(format!(
                        "Failed to update workspace context clip: {err}"
                    ));
                }
            }
            WorkspaceDashboardAction::ConvertAgentResultToProposal { result_id } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .convert_agent_result_to_proposal(app_server, result_id)
                        .await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget.add_error_message(format!(
                        "Failed to convert workspace agent result: {err}"
                    ));
                }
            }
            ...
            WorkspaceDashboardAction::SignNote => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard.sign_current_note(app_server).await
                } else {
                    Ok(())
                };
                if let Err(err) = result {
                    self.chat_widget
                        .add_error_message(format!("Failed to sign workspace note: {err}"));
                }
            }
            WorkspaceDashboardAction::UpdateTaskStatus { task_id, status } => {
                let result = if let Some(dashboard) = self.workspace_dashboard.as_mut() {
                    dashboard
                        .update_task_status(app_server, task_id, status)
                        .await
                } else {
                    Ok(())
                };
    ...
    fn workspace_dashboard_active(&self) -> bool {
        self.workspace_dashboard_visible && self.workspace_dashboard.is_some()
    }

    fn hide_workspace_dashboard(&mut self) {
    ...
    fn render_workspace_dashboard_frame(
        ...
        tui.draw_with_resize_reflow(desired_height, |frame| {
            let area = frame.area();
            rendered_area = area;
            if let Some(dashboard) = &self.workspace_dashboard {
                dashboard.render(area, frame.buffer);
                if let Some((x, y)) = dashboard.cursor_pos(area) {
                    frame.set_cursor_style(dashboard.cursor_style());
                    frame.set_cursor_position((x, y));
                }
            }
        })?;
