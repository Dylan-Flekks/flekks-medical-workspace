...
async fn workspace_close_preserves_unsaved_dashboard_draft() {
    ...
    assert_eq!(
        app.workspace_dashboard
            .as_ref()
            .expect("dashboard should still be available")
            .note_body_for_tests(),
        "unsaved note text"
    );
}
...
#[tokio::test]
async fn medical_workspace_close_preserves_unsaved_dashboard_draft() {
    let mut app = make_test_app().await;
    let mut dashboard = WorkspaceDashboard::new(WorkspaceProfile::Medical);
    dashboard.set_note_body_for_tests("unsaved medical note text");
    app.workspace_dashboard = Some(dashboard);
    app.workspace_dashboard_visible = true;
...
#[test]
fn medical_workspace_context_bridge_inserts_structured_prompt_and_returns_to_agent_mode()
-> Result<()> {
    run_workspace_dashboard_runtime_test(Box::pin(async {
        let mut app = make_test_app().await;
        let codex_home = tempdir()?;
        ...
        assert!(composer.contains("Rendered packet context snapshot"));
        ...
        assert!(composer.contains("Context packet safety"));
        assert!(composer.contains("Alex Example"));
        assert!(composer.contains("Visit note"));
        assert!(composer.contains("Patient reports improved sleep."));
        app_server.shutdown().await?;
        Ok(())
    }))
}
...
fn medical_workspace_first_eval_round_trip_stays_packet_scoped_and_review_pending() -> Result<()> {
    run_workspace_dashboard_runtime_test(Box::pin(async {
        ...
        assert!(composer.contains("Agent-visible packet handle"));
        assert!(composer.contains("workspace/context/packet/replay"));
        assert!(composer.contains("Riley Testpatient"));
        assert!(composer.contains("First eval"));
        assert!(composer.contains("HPI: Fake patient reports two weeks of mild cough."));
...
#[test]
fn workspace_dashboard_recovers_rpc_checkpoint_without_mutating_canonical_note() -> Result<()> {
    run_workspace_dashboard_runtime_test(Box::pin(async {
        let mut app = make_test_app().await;
        let codex_home = tempdir()?;
        app.config.codex_home = codex_home.path().to_path_buf().abs();
        ...
        let mut recovery_server =
            Box::pin(crate::start_embedded_app_server_for_picker(&app.config)).await?;
        let mut recovered = WorkspaceDashboard::new(WorkspaceProfile::Medical);
        recovered.load(&mut recovery_server).await?;
        assert_eq!(recovered.note_body_for_tests(), "Canonical human note.");
        assert!(recovered.has_pending_draft_recovery_for_tests());
        assert_eq!(
            recovered.handle_key_event(KeyEvent::from(KeyCode::Char('r'))),
            WorkspaceDashboardAction::RestoreDraftCheckpoint
        );
        recovered
            .restore_pending_draft(&mut recovery_server)
            .await?;
        assert_eq!(
            recovered.note_body_for_tests(),
            "Recovered local human note."
        );
        recovered.save(&mut recovery_server).await?;
        recovered
            .close_draft_after_canonical_save(&mut recovery_server)
            .await?;
        ...
        final_dashboard.load(&mut final_server).await?;
        assert_eq!(
            final_dashboard.note_body_for_tests(),
            "Recovered local human note."
        );
        assert!(!final_dashboard.has_pending_draft_recovery_for_tests());
    ...
    run_workspace_dashboard_runtime_test(Box::pin(async {
        ...
        let patient = app_server
            .workspace_client_upsert(WorkspaceClientUpsertParams {
                ...
                sex_or_gender: Some("unknown".to_string()),
                external_id: Some("FAKE-MEDICARE-MBI-001".to_string()),
                record_start_date: Some("2026-06-01".to_string()),
                record_end_date: None,
                summary: "Contact: fake mobile 555-0100; emergency contact: fake spouse 555-0101; Medicare: fake Part B active; pharmacy: fake local pharmacy.".to_string(),
                ..Default::default()
            })
            .await?
            .client;
...
fn workspace_context_packet_replay_stays_historical_after_daily_chart_changes() -> Result<()> {
    run_workspace_dashboard_runtime_test(Box::pin(async {
        ...
        let envelope = serde_json::json!({
            ...
            "patient": {
                "displayName": "Riley PacketHistory",
                "externalId": "OLD_FAKE_MEDICARE_SENTINEL",
                "summary": "OLD_CONTACT_SENTINEL; OLD_MEDICARE_SENTINEL",
            },
            "activeNote": {
                "title": "Follow-up visit",
                "body": "OLD_NOTE_BODY_SENTINEL: draft plan before admin update.",
            },
            "selectedArtifacts": [],
            "selectedDerivatives": [],
            "selectedClips": [],
