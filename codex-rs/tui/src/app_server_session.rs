...
impl AppServerSession {
    ...
    pub(crate) async fn workspace_client_list(&mut self) -> Result<WorkspaceClientListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceClientList {
                request_id,
                params: WorkspaceClientListParams {},
            })
            .await
    ...
    pub(crate) async fn workspace_note_list(
        &mut self,
        client_id: String,
    ) -> Result<WorkspaceNoteListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceNoteList {
                request_id,
                params: WorkspaceNoteListParams { client_id },
            })
            .await
    ...
    pub(crate) async fn workspace_note_upsert(
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/note/upsert failed in TUI")
    }

    pub(crate) async fn workspace_note_sign(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/note/sign failed in TUI")
    }

    pub(crate) async fn workspace_note_signature_list(
        &mut self,
        note_id: String,
        ...
        self.client
            .request_typed(ClientRequest::WorkspaceNoteSignatureList {
                request_id,
                params: WorkspaceNoteSignatureListParams { note_id },
            })
            .await
            .wrap_err("workspace/note/signature/list failed in TUI")
    }

    pub(crate) async fn workspace_note_addendum_create(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/note/addendum/create failed in TUI")
    }

    pub(crate) async fn workspace_note_addendum_list(
        &mut self,
        note_id: String,
    ) -> Result<WorkspaceNoteAddendumListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceNoteAddendumList {
                request_id,
                params: WorkspaceNoteAddendumListParams { note_id },
            })
            .await
            .wrap_err("workspace/note/addendum/list failed in TUI")
    }

    pub(crate) async fn workspace_note_proposal_list(
        &mut self,
        note_id: String,
    ) -> Result<WorkspaceNoteProposalListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceNoteProposalList {
                request_id,
                params: WorkspaceNoteProposalListParams { note_id },
            })
            .await
            .wrap_err("workspace/note/proposal/list failed in TUI")
    }

    pub(crate) async fn workspace_note_proposal_create(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/note/proposal/create failed in TUI")
    }

    pub(crate) async fn workspace_note_proposal_resolve(
        &mut self,
    ...
    pub(crate) async fn workspace_encounter_list(
        &mut self,
        client_id: String,
    ) -> Result<WorkspaceEncounterListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceEncounterList {
                request_id,
                params: WorkspaceEncounterListParams { client_id },
            })
            .await
            .wrap_err("workspace/encounter/list failed in TUI")
    }

    pub(crate) async fn workspace_encounter_upsert(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/encounter/upsert failed in TUI")
    }

    pub(crate) async fn workspace_audit_list(
        &mut self,
        params: WorkspaceAuditListParams,
    ) -> Result<WorkspaceAuditListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceAuditList { request_id, params })
            .await
            .wrap_err("workspace/audit/list failed in TUI")
    }

    pub(crate) async fn workspace_document_list(
        &mut self,
        client_id: String,
        ...
        self.client
            .request_typed(ClientRequest::WorkspaceDocumentList {
                request_id,
                params: WorkspaceDocumentListParams { client_id },
            })
            .await
            .wrap_err("workspace/document/list failed in TUI")
    }

    pub(crate) async fn workspace_practice_library_list(
        &mut self,
        params: WorkspacePracticeLibraryListParams,
    ) -> Result<WorkspacePracticeLibraryListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspacePracticeLibraryList { request_id, params })
            .await
            .wrap_err("workspace/practice/library/list failed in TUI")
    }

    pub(crate) async fn workspace_document_upsert(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/document/upsert failed in TUI")
    }

    pub(crate) async fn workspace_patient_safety_item_list(
        &mut self,
        client_id: String,
    ) -> Result<WorkspacePatientSafetyItemListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspacePatientSafetyItemList {
                request_id,
                params: WorkspacePatientSafetyItemListParams { client_id },
            })
            .await
    ...
    ) -> Result<WorkspaceArtifactDerivativeListResponse> {
        let request_id = self.next_request_id();
        self.client
    ...
    pub(crate) async fn workspace_artifact_derivative_status_update(
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/artifact/derivative/status/update failed in TUI")
    }

    pub(crate) async fn workspace_context_clip_list(
        &mut self,
        params: WorkspaceContextClipListParams,
    ) -> Result<WorkspaceContextClipListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceContextClipList { request_id, params })
            .await
    ...
    pub(crate) async fn workspace_context_clip_status_update(
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/context/clip/status/update failed in TUI")
    }

    pub(crate) async fn workspace_task_list(
        &mut self,
        client_id: String,
    ) -> Result<WorkspaceTaskListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceTaskList {
                request_id,
                params: WorkspaceTaskListParams { client_id },
            })
            .await
    ...
    pub(crate) async fn workspace_task_upsert(
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/task/upsert failed in TUI")
    }

    pub(crate) async fn workspace_task_status_update(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/task/status/update failed in TUI")
    }

    pub(crate) async fn workspace_context_packet_list(
        &mut self,
        params: WorkspaceContextPacketListParams,
    ) -> Result<WorkspaceContextPacketListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceContextPacketList { request_id, params })
            .await
            .wrap_err("workspace/context/packet/list failed in TUI")
    }

    pub(crate) async fn workspace_context_packet_create(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/context/packet/create failed in TUI")
    }

    #[cfg(test)]
    pub(crate) async fn workspace_context_packet_replay(
        &mut self,
    ...
    ) -> Result<WorkspaceAgentResultListResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceAgentResultList { request_id, params })
            .await
            .wrap_err("workspace/agent/result/list failed in TUI")
    }

    pub(crate) async fn workspace_agent_result_create(
        &mut self,
        ...
        self.client
            ...
            .await
            .wrap_err("workspace/agent/result/create failed in TUI")
    }

    pub(crate) async fn workspace_agent_result_status_update(
        &mut self,
        params: WorkspaceAgentResultStatusUpdateParams,
    ) -> Result<WorkspaceAgentResultStatusUpdateResponse> {
        let request_id = self.next_request_id();
        self.client
    ...
    #[cfg(test)]
    pub(crate) async fn workspace_context_get(
        &mut self,
        params: WorkspaceContextGetParams,
    ) -> Result<WorkspaceContextGetResponse> {
        let request_id = self.next_request_id();
        self.client
            .request_typed(ClientRequest::WorkspaceContextGet { request_id, params })
            .await
            .wrap_err("workspace/context/get failed in TUI")
    }
