    ...
    fn map_crossterm_event(&mut self, event: Event) -> Option<TuiEvent> {
        match event {
            ...
            Event::Mouse(mouse_event) => match mouse_event.kind {
                MouseEventKind::ScrollUp => Some(TuiEvent::MouseScroll(MouseScrollEvent {
...
mod tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn mouse_scroll_maps_to_scroll_event() {
        let (broker, handle, _draw_tx, draw_rx, terminal_focused) = setup();
        let mut stream = make_stream(broker, draw_rx, terminal_focused);
        ...
        match stream.next().await {
            Some(TuiEvent::MouseScroll(event)) => {
                assert_eq!(event.direction, MouseScrollDirection::Down);
                assert_eq!(event.column, 12);
                assert_eq!(event.row, 7);
            }
            other => panic!("expected mouse scroll event, got {other:?}"),
        }
    }

    #[tokio::test(flavor = "current_thread")]
    async fn unmapped_mouse_events_are_skipped() {
        let (broker, handle, _draw_tx, draw_rx, terminal_focused) = setup();
        let mut stream = make_stream(broker, draw_rx, terminal_focused);

        let expected_key = KeyEvent::new(KeyCode::Char('m'), KeyModifiers::NONE);
        handle.send(Ok(Event::Mouse(MouseEvent {
        ...
        match stream.next().await {
            ...
            other => panic!("expected key after skipped mouse event, got {other:?}"),
        }
    }
