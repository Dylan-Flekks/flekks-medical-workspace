...
pub enum TuiEvent {
    ...
    Paste(String),
    /// A mouse or trackpad wheel event.
...
impl Tui {
    ...
    /// Enter alternate screen for a full-screen workspace surface.
    ///
    /// The medical workspace is not usable as inline scrollback: macOS trackpad
    /// scroll can expose previous full-screen renders as stacked copies. Keep
    /// the global chat preference separate from this full-screen surface.
    pub fn enter_workspace_alt_screen(&mut self) -> Result<()> {
        self.enter_alt_screen_impl()
    }

    fn enter_alt_screen_impl(&mut self) -> Result<()> {
