...
use crate::WORKSPACE_DB_FILENAME;
...
use crate::migrations::runtime_workspace_migrator;
...
mod workspace_packet_checkpoint;
...
const WORKSPACE_DB: RuntimeDbSpec = RuntimeDbSpec {
    label: "workspace DB",
    filename: WORKSPACE_DB_FILENAME,
    kind: DbKind::Workspace,
    open_phase: "open_workspace",
    migrate_phase: "migrate_workspace",
};
...
impl StateRuntime {
    ...
    async fn init_inner(
        ...
        let workspace_migrator = runtime_workspace_migrator();
        ...
        let workspace_path = WORKSPACE_DB.path(codex_home.as_path());
        ...
        let workspace_pool =
            match open_workspace_sqlite(&workspace_path, &workspace_migrator, telemetry_override)
                .await
            {
                Ok(db) => Arc::new(db),
                Err(err) => {
                    warn!(
                        "failed to open workspace db at {}: {err}",
        ...
        crate::telemetry::record_init_result(
            ...
            started.elapsed(),
            &client_admin_backfill_result,
        );
        if let Err(err) = client_admin_backfill_result {
            close_sqlite_pools(&[
                pool.as_ref(),
                logs_pool.as_ref(),
                goals_pool.as_ref(),
    ...
    pub async fn close(&self) {
        self.workspace.pool.close().await;
...
async fn open_workspace_sqlite(
    path: &Path,
    migrator: &Migrator,
    telemetry_override: Option<&dyn DbTelemetry>,
) -> anyhow::Result<SqlitePool> {
    open_sqlite(path, migrator, WORKSPACE_DB, telemetry_override).await
}
...
pub fn workspace_db_filename() -> &'static str {
    WORKSPACE_DB.filename
}

pub fn workspace_db_path(codex_home: &Path) -> PathBuf {
    WORKSPACE_DB.path(codex_home)
}
