...
impl WorkspaceContextPacketRow {
    pub(crate) fn try_from_row(row: &SqliteRow) -> Result<Self> {
        Ok(Self {
            ...
            workspace_profile: row.try_get("workspace_profile")?,
            plan_schema_version: row.try_get("plan_schema_version")?,
            source_checkpoint_id: row.try_get("source_checkpoint_id")?,
            source_checkpoint_sha256: row.try_get("source_checkpoint_sha256")?,
            readiness_json: row.try_get("readiness_json")?,
