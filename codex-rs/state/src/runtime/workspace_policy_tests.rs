...
async fn every_workspace_domain_table_blocks_api_and_direct_provisioning() {
    for table in WORKSPACE_DOMAIN_TABLES {
        // Use a fresh database for every table. Ordered coverage intentionally rejects deletion
        // and mirrors its primary row into the legacy projection, so cleanup-based isolation can
        // itself exercise production triggers instead of the policy boundary under test.
        let runtime = runtime().await;
