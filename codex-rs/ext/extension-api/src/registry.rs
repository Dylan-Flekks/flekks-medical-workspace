...
pub struct ExtensionRegistryBuilder<C: Sync> {
    ...
    workflow_contributors: Vec<Arc<dyn WorkflowContributor>>,
...
impl<C: Sync> ExtensionRegistryBuilder<C> {
    ...
    /// Registers one regulated-workflow state contributor.
    pub fn workflow_contributor(&mut self, contributor: Arc<dyn WorkflowContributor>) {
        self.workflow_contributors.push(contributor);
    }
    ...
    pub fn build(self) -> ExtensionRegistry<C> {
        ExtensionRegistry {
            ...
            workflow_contributors: self.workflow_contributors,
        }
    }
}
...
impl<C: Sync> ExtensionRegistry<C> {
    ...
    /// Returns the registered regulated-workflow state contributors.
    pub fn workflow_contributors(&self) -> &[Arc<dyn WorkflowContributor>] {
