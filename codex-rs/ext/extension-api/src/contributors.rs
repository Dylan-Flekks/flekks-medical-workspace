...
pub type WorkflowSnapshotFuture<'a> =
    Pin<Box<dyn Future<Output = Option<WorkflowSnapshot>> + Send + 'a>>;
...
/// Extension contribution that reports redacted regulated-workflow state.
///
/// Implementations should return a domain-neutral snapshot suitable for
/// durable session history and UI display. Raw PHI, secrets, credentials, and
/// full tool payloads do not belong in workflow snapshots.
pub trait WorkflowContributor: Send + Sync {
    fn snapshot<'a>(
        &'a self,
        session_store: &'a ExtensionData,
        thread_store: &'a ExtensionData,
    ) -> WorkflowSnapshotFuture<'a>;
}
